...
- **FR-4.** Each `ProofRef` MUST carry: `amount`, `keyset_id`, the deterministic identifier `Y = hash_to_curve(secret)` (33-byte compressed point, hex-encoded), and the privacy-mode-dependent fields per Section 7.2. The canonical primary key for proof references, storage indexes, and DAG joins is the tuple `(mint_url, keyset_id, Y)` — never `Y` alone, and never `(keyset_id, Y)` without `mint_url`. Although `Y` is overwhelmingly likely to be unique on its own under current Cashu cryptography (`secp256k1` + `hash_to_curve`), future NUTs may introduce alternative curves or hashes, and two mints may advertise colliding 8-byte keyset ids; including `mint_url` guards against both and matches the edge-derivation rule (FR-5) and the §5.4 `input_y` / `output_y` index design. Storage indexes and walk algorithms MUST therefore key on the full tuple; `Y` alone (optionally with `keyset_id`) is accepted only as a client convenience in URL paths, where the implementation looks up against the full tuple internally and returns grouped/disambiguated matches per mint and keyset.
...
**Issuer-less event back-fill (decided).** A producer that publishes a `MINT` or `SWAP` before the corresponding voucher is issued cannot include `issuer_id` / `issuer_pubkey` at publish time. The spec adopts **option (c)**: a sidecar mapping `(mint_url, keyset_id, Y) → issuer` is maintained in the SQLite index and back-filled when the voucher event later arrives, without emitting a Nostr correction event. The mapping key is the full proof-identity tuple `(mint_url, keyset_id, Y)` — never `(keyset_id, Y)` alone — so that two mints whose 8-byte keyset ids collide cannot have issuer data misattributed across them (the same isolation rationale §5.4 applies to the `input_y` / `output_y` indexes). Read responses where the issuer fields originate from the back-fill sidecar (rather than the raw event tags) MUST include the annotation `issuerProvenance: "index_backfill"` (versus the default `issuerProvenance: "event_tag"`) so consumers can audit the source. The other two options were rejected: (a) require callers to follow `voucher_ref` at read time — adds a round-trip per result, defeating the denormalisation in §5.3.1; (b) emit a `correction_of` event when the voucher arrives — doubles event volume for every issuer-less mint/swap and clutters the audit log with mechanical corrections. Option (c) preserves event volume but the raw signed event no longer matches the indexed query result; the explicit `issuerProvenance` annotation makes that divergence auditable rather than hidden.

**Back-fill mechanics.** When a kind-30078 voucher event arrives carrying proofs (via `parentContributions` or its child voucher chain), the voucher-state watcher (§6.5) extracts `(mint_url, keyset_id, Y) → (issuer_id, issuer_pubkey)` mappings and writes them to a `proof_issuer_backfill` sidecar table. The events index is then updated for any rows whose `(mint_url, keyset_id, Y)` matches and whose `issuer_id` / `issuer_pubkey` are NULL. The raw nostrdb event is NOT modified — only the sidecar — so signature validity is preserved. The back-fill operation is logged at INFO with `issuer_backfill_completed proof_count=N voucher_id=<v-id>` per voucher.

**Re-index on disagreement.** If a back-filled `(mint_url, keyset_id, Y)` later receives a corrected voucher with a different issuer (rare but possible if the operator rebinds proofs), the sidecar is updated and a WARN is logged with `issuer_backfill_overwrite previous=<old> new=<new>`. The history of back-fill changes is NOT retained; consumers that need the full chain follow the `voucher_ref` link.

**Pre-voucher issuer exposure control (decided).** Section 5.2 exempts `issuer_id` / `issuer_pubkey` from redaction on the premise that the voucher event already publishes them in plaintext. That premise does not hold for an event published **before** its voucher is bound (`MINT` / `SWAP` ahead of issuance, or any merchant-tagged event without a `voucher_ref`): exposing the issuer on such an event can reveal merchant→activity correlation earlier than the voucher ledger itself would. The ledger MUST therefore provide a deployment-configurable control governing issuer exposure on not-yet-bound events, `trace.issuer.pre-voucher-exposure`, with two modes:

- `suppress` — issuer tags are withheld from read responses on any event that has no `voucher_ref` (and no back-fill mapping yet); they appear once the voucher binds (via `voucher_ref` or the back-fill sidecar above).
- `summary-withhold` (default) — issuer fields are returned to `trace:read:hashed` and `trace:read:full` callers but withheld from `trace:read:summary` callers until a `voucher_ref` (or back-fill mapping) is present.

Once an event is bound to a voucher, the Section 5.2 exemption applies normally and issuer fields are returned verbatim to all read authorities. This control affects read-time serialisation only; it never alters the stored event or the back-fill sidecar. See Section 7.2.
...
- **Issuer fields are not privacy-sensitive at the ledger boundary — once the event is voucher-bound.** `issuer_id` and `issuer_pubkey` are returned verbatim to all read authorities (`trace:read:summary`, `trace:read:hashed`, `trace:read:full`) for any event that carries a `voucher_ref` (or a back-fill mapping), because the same values are already public in the underlying voucher event. Operators concerned about exposing merchant→activity correlation to summary-only auditors should restrict access to the trace API at the authority level rather than rely on field-level redaction. This is a deliberate departure from the otherwise privacy-mode-driven serialisation in Section 5.2. **Before** an event is voucher-bound, the `trace.issuer.pre-voucher-exposure` control (§5.3.1 — Pre-voucher issuer exposure control) governs whether issuer fields are suppressed entirely or merely withheld from summary readers, because in that window the voucher ledger has not yet published the issuer and the trace event would otherwise leak it early.
...
| ID    | Task                                                                                  | Size | Depends On  | Status  | Commit |
|-------|---------------------------------------------------------------------------------------|------|-------------|---------|--------|
| T3.1  | Implement `TraceEventStore` (nostrdb + SQLite sidecar) with kind-9079 indexes          | L    | T1.x        | Done    | dec818a, 7dc4c3b — SQLite sidecar + IndexedTraceEventStore + nostrdb-backed RawEventStore adapter with startup rebuild |
...
### Round 18 — Spec-generation review fixes

A review of the generated feature spec (`specs/001-transaction-traceability/`) surfaced fixes that are folded back here so the design and the generated spec stay consistent. Two are normative design changes; the remainder are spec-level clarifications already consistent with this design and noted for traceability.

**Normative design changes (made above):**

- [Section 4.1 — FR-4] The canonical primary key for proof references, storage indexes, and DAG joins is changed from `(keyset_id, Y)` to the full proof-identity tuple `(mint_url, keyset_id, Y)`, with `Y` alone (optionally `+ keyset_id`) accepted only as a client convenience that returns grouped/disambiguated matches. Rationale: FR-5 edge derivation and the §5.4 indexes already key on `(mint_url, keyset_id, Y)`; leaving FR-4 at `(keyset_id, Y)` created an avoidable ambiguity for implementers.
- [Section 5.3.1 — Issuer back-fill] The back-fill mapping key is changed from `(keyset_id, Y)` to the full proof-identity tuple `(mint_url, keyset_id, Y)` in all three subsections ("Issuer-less event back-fill", "Back-fill mechanics", "Re-index on disagreement"), and the corresponding closed-question (§10 #12) and Round 17 references. Rationale: `(keyset_id, Y)` alone can misattribute issuer data across two mints whose 8-byte keyset ids collide; the full tuple matches the isolation already applied to the §5.4 `input_y` / `output_y` indexes.
- [Section 5.3.1 — new "Pre-voucher issuer exposure control"; Section 7.2] Added a deployment-configurable control (`trace.issuer.pre-voucher-exposure`: `suppress` | `summary-withhold` (default)) governing issuer-field exposure on events that are not yet voucher-bound. Rationale: the §5.2 "issuer is already public" exemption only holds once the voucher is published; a `MINT`/`SWAP` emitted ahead of issuance would otherwise leak merchant→activity correlation earlier than the voucher ledger does. Read-time only; never alters stored events.

**Spec-level clarifications (in the generated spec; consistent with this design, no design change required):**

- Raw-secret scope: "no raw secret reaches the ledger" applies to **read/query endpoints** only; `FULL`-mode ingest legitimately carries raw proof payloads over the private authenticated transport (§7.2). The generated spec's SC/US wording was tightened to say so.
- Bundle-token equality: SEND/RECEIVE `bundleToken`, when present, MUST equal the event `inputs` **exactly** (mint URL, unit, ordered `(keyset_id, amount, secret, C)`) per §5.10 — restating the existing `B1_BUNDLE_MISMATCH` rule against a "superset" misreading.
- Proof lookup disambiguation: `/proofs/{y}` and `/proofs/{y}/walk` accept optional `mintUrl`/`keysetId`, group history by `(mint_url, keyset_id, Y)` when omitted, and return `409 AMBIGUOUS_PROOF` for an under-specified walk on a `Y` that matches multiple mints/keysets (consistent with FR-4's tuple key).
- Producer-owned reconciliation: completeness reconciliation lives in the producer SDK (only the producer knows the domain operations that should have emitted events). The ledger's role is to expose observable counters/status for the reconciler to compare against — accepted events, duplicates, rejected-by-reason, per-producer lag, and lookup by `operation_id` — surfaced via `/stats` (§5.5) and the FR-11 stuck-events view (§4.2).
- Overflow framing: the "never degrade" goal is steady-state; under sustained overflow the default `BLOCK_AND_ALERT` policy (§4.2 FR-11a) adds a bounded block then surfaces an observable failure — never a silent drop.
- Security gates are not deferred: NIP-98 auth, access-level shaping (§7.3), private-relay enforcement (§7.2), and read/payload-reveal audit logging (§7.4) are v1/foundational, not optional hardening.
